...
const READY = "TANCMARK_GITHUB_PUBLIC_RELEASE_WITH_FULL_SECURE_DEMO_V12";
...
export function loadReleaseStateDocuments(root = DEFAULT_ROOT, privateLedgerPath = process.env.TANCMARK_PRIVATE_SECURITY_RECONCILIATION) {
  const documents = {
    ...
    finalRelease: readJson(root, "reports/PUBLIC_FINAL_RELEASE_RESULT_20260831.json"),
...
export function evaluateReleaseStateDocuments(documents) {
  ...
  if (closure?.releaseDecision?.releaseBlockersRemaining !== 0 || closure?.releaseDecision?.status !== READY) fail("SECURITY_CLOSURE_RELEASE_DECISION_MISMATCH");
