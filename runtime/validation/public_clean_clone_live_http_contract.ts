...
async function cleanupTempRoot(): Promise<void> {
  if (tempCleanupHandled) return;
  tempCleanupHandled = true;
...
async function main(): Promise<void> {
  const [{ generateLiveFmp4Fixture }, { shutdownLiveLocalRuntime }] = await Promise.all([
    import("./live_local_product_media_fixture.ts"),
    import("../../artifacts/api-server/src/live/liveLocalRuntime.ts"),
  ]);
...
try {
  ...
  for (let sequence = 2; sequence < fixture.fragments.length; sequence += 1) {
    const fragment = fixture.fragments[sequence] as Buffer;
    const response = await fetch(`${base}/sessions/${sessionId}/segments`, { method: "POST", headers: { ...auth, "content-type": "application/octet-stream", "x-content-sha256": sha(fragment), "x-idempotency-key": `http-segment-key-${String(sequence + 1).padStart(4, "0")}`, "x-segment-sequence": String(sequence), "x-segment-duration-ms": String(fixture.durationsMs[sequence]) }, body: fragment });
    assert.equal(response.status, 201, JSON.stringify(await json(response.clone())));
  ...
  const stop = await postJson(`${base}/sessions/${sessionId}/stop`, { expectedRevision: session.revision }, { "x-idempotency-key": "http-stop-key-0001" }); const stopped = await json(stop); assert.equal(stop.status, 200, JSON.stringify(stopped)); session = stopped.session; assert.equal(session.status, "STOPPED"); assert.equal(session.finalVerificationState, "EXACT_VERIFIED"); assert.equal(stopped.finalVerification.verdict, "VIDEO_LAYER_VAULT"); assert.equal(stopped.evidence.ownership, true); assert.equal(stopped.evidence.vault, true); assert.equal(stopped.evidence.final, true);
  ...
  console.log(JSON.stringify({ contract: "public_clean_clone_live_http_contract", status: "passed", loopbackHost: "127.0.0.1", trueHttpE2e: true, actualSocketTransportBoundary: { loopbackPlaintextAccepted: true, nonLoopbackPlaintextRejected: true, forwardedHeadersIgnored: true }, managementConsole: "FUNCTIONAL_NO_PERSISTED_SECRETS", sourceKind: "STRUCTURAL_SYNTHETIC_FMP4", privateMediaUsed: false, sourcePathDisclosed: false, sourceHashDisclosed: false, uploadedFragmentCount: fixture.fragments.length, realPhysicalVideoWatermarking: true, llHlsPartsUnder500ms: fixture.durationsMs.every((value) => value <= 500), authTenantSpoofNegatives: true, noClientSuppliedExactId: true, cookieOnlyExchange: true, recordingLeastPrivilegeScope: true, replayAndRevoke: true, runningAndStoppedVod: true, automaticFinalExactVerification: true, finalVault: true, etag304: true, ffprobeRecording: true, cleanShutdown: true, externalNetworkCalls: 0 }, null, 2));
} finally {
  server.closeAllConnections();
  await new Promise<void>((resolve) => server.close(() => resolve()));
  await shutdownLiveLocalRuntime();
  await cleanupTempRoot();
}
}
void main().catch(async (error) => {
  try { await cleanupTempRoot(); } catch (cleanupError) { console.error(cleanupError); }
  console.error(error);
  process.exitCode = 1;
});
